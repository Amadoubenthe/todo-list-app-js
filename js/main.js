...
function eventListeners() {
  ...
  //Remove tasks from checkList
  checkList.addEventListener('click', removeTask);
...
function completeTask(e) {
  if (e.target.classList.contains('checked-task')) {
    ...
    //Create the Date
    const completedAt = document.createElement('a');
    completedAt.classList = 'completed-at';
    let d = new Date();
    let datestring = d.getDate()  + "-" + (d.getMonth()+1) + "-" + d.getFullYear() + " " +
    d.getHours() + ":" + d.getMinutes();
    completedAt.textContent = datestring;
